...
export default function BinderPicker({ onSelect, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
      <div className="bg-dbb-secondary border border-gray-700 rounded-xl shadow-2xl w-full max-w-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700">
          <h2 className="text-base font-semibold text-white">Move to binder</h2>
        ...
        <div className="p-3 max-h-80 overflow-y-auto">
          ...
          {!loading && binders.map(b => (
            <button
              key={b.id}
              onClick={() => onSelect(b)}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-dbb-primary/60 transition-colors text-left"
            >
        ...
        <div className="px-5 py-4 border-t border-gray-700">
          <button
            onClick={onClose}
            className="w-full text-sm text-gray-400 hover:text-white border border-gray-700 hover:border-gray-500 rounded-lg py-2 transition-colors"
          >
            Cancel
          </button>
        </div>
