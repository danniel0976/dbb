...
async function checkoutResult(sc, buyerId, orderIds, idempotentReplay) {
  const { data: orders, error } = await sc.from('orders').select(`id, buyer_id, seller_id, pickup_location_id, status, currency, total_myr, created_at, pickup_locations(name, address, operating_notes), order_items(id, quantity, unit_myr, line_myr, card_name, set_code, set_name, collector_number, finish, condition)`).in('id', orderIds).eq('buyer_id', buyerId).order('created_at', { ascending: true })
  ...
  const { data: profiles, error: profileError } = await sc.from('profiles').select('id, display_name, merchant_bank_name, merchant_account_name, merchant_account_number, merchant_duitnow_id, merchant_payment_instructions, merchant_bank_qr_path, merchant_tng_qr_path, merchant_profile_completed_at').in('id', sellerIds)
  ...
  for (const profile of profiles || []) {
    ...
    paymentBySeller.set(profile.id, { seller_name: profile.display_name || 'Seller', bank_name: profile.merchant_bank_name, account_name: profile.merchant_account_name, account_number: profile.merchant_account_number, duitnow_id: profile.merchant_duitnow_id, payment_instructions: profile.merchant_payment_instructions, bank_qr_url: bankQrUrl, tng_qr_url: tngQrUrl, qr_expires_in_seconds: QR_TTL_SECONDS })
  }
...
function statusOf(code) { if (code === 'NOT_WINNER') return 403; if (code === 'AUCTION_NOT_FOUND') return 404; if (code === 'AUCTION_ENDED' || code === 'CLAIM_WINDOW_EXPIRED') return 410; return 409 }
...
export async function POST(request, { params }) {
  ...
  const { data, error } = await sc.rpc('checkout_auction_claim', { p_winner_id: user.id, p_idempotency_key: body.idempotency_key, p_pickup_location_id: body.pickup_location_id, p_auction_id: id })
  if (error) {
    const code = codeOf(error)
    return privateJson({ error: code }, { status: statusOf(code) })
  }
  if (data?.result_code === 'CLAIM_WINDOW_EXPIRED') return privateJson({ result_code: data.result_code, error: 'Claim window has expired' }, { status: 410 })
  if (data?.result_code !== 'CHECKOUT_COMPLETE') return privateJson({ error: 'Checkout returned an unexpected result' }, { status: 500 })
  try { return privateJson(await checkoutResult(sc, user.id, data.order_ids || [], Boolean(data.idempotent_replay)), { status: 201 }) } catch (error) { console.error('[POST /api/auctions/:id/claim]', error.message); return privateJson({ error: 'Orders were created, but payment details could not be displayed.' }, { status: 500 }) }
}
